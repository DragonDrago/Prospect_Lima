<template>
  <div class="sales page">
    ...
    >
      <template #name>{{ $t("list_of_pharmacies") }}</template>
    </the-filter>
    <div class="statistics-menu statistic-visit">
      ...
      <div class="statistics-menu__card">
        ...
        <div class="statistics-menu__content">
          ...
          <div class="statistics-menu__num">
            {{ prettify(getTotal("prepayment_sum")) }}
          </div>
        </div>
      </div>
      <div class="statistics-menu__card">
    ...
    <div v-else>
      <the-table
        ...
        class="list-pharmacy-table"
      >
        <tr v-for="b in drugStores" :key="b.id">
          <td>{{ b.name }}</td>
          <td>{{ b.inn || $t("empty") }}</td>
          <td>{{ b.address }}</td>
          <td>{{ b.sale_count }}</td>
      ...
      <empty v-else v-show="isEmpty" :item="drugStores" />
    </div>
    <modal
    ...
    >
      <template #title>{{ $t("add_a_pharmacy") }}</template>
    ...
    <teleport to="body">
      <modal
        ...
        @submitHandler="filterPharmacy"
      >
        <template #title>{{ $t("filter_list_pharmacy") }}</template>
...
export default {
  ...
  computed: {
    ...mapGetters("drugStores", ["drugStores", "isLoading", "pages"]),
  ...
  methods: {
    ...
    getTotal(key) {
      return this.drugStores.reduce((acc, val) => acc + val[key], 0);
    },
    changeHandler(e) {
      const { name, value } = e.target;
      this.drugStore = { ...this.drugStore, [name]: value };
    },
    selectBranch(event) {
    ...
    async filterPharmacy() {
      ...
      this.filterItems = Object.values(this.drugStore);
      await this.filterDrugStores(this.drugStore);
    },
    async removeFilterItem(item) {
      this.filterItems = this.filterItems.filter((d) => d !== item);
      for (let key in this.drugStore) {
        if (this.drugStore[key].includes(item)) {
          delete this.drugStore[key];
        }
      }
      await this.filterDrugStores(this.drugStore);
    },
    editHandler(drugStore) {
      this.s.isEditOpen = true;
      setTimeout(() => {
        this.$refs.inputValue.value = drugStore.address;
      }, 200);
      this.selectBranch({ target: { value: drugStore.region_id } });
  ...
  async mounted() {
    ...
    await this.getDrugStores(this.currentPage - 1);
